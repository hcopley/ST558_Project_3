...
```{r compare_logloss_on_test, eval=FALSE}
# stack model objects into a list
models <- list(log_reg_1 = log_reg_1,
               log_reg_2 = log_reg_3,
               log_reg_2 = log_reg_3,
               tree = tree,
...
The model with the highest accuracy is the `r substr(names(which.min(perf)),1,nchar(names(which.min(perf)))-8)` model with an accuracy = `r round(min(perf),3)`.
